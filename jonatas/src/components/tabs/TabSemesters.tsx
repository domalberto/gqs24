...
export default function TabSemesters() {
  ...
  return (
    <>
      ...
      whileInView={"visible"}
      className="bg-branco_1 rounded-lg shadow-xl md:col-span-12 col-span-2 row-span-3 2xl:col-span-8 md:row-span-3 p-10">
        <h2 className="text-center text-2xl text-azul_3">Grade de disciplinas</h2>
          ...
          <section className="flex justify-evenly max-md:overflow-y-scroll p-4 mt-6 mb-4">
            {Array.from({ length: 5 }, (_, i) => (
              <button
                ...
                className="h-13 min-w-32 w-1/5 mx-1 text-azul px-6 py-4 hover:bg-branco_2 hover:text-gray-800 transition-all ease-in-out delay-50 rounded-lg"
              >
                Semestre {i + 1}
              </button>
            ))}
            ...
            {semestreData.map(
              (semestreData) =>
                ...
                semestreData.disciplinas.map((disciplina) => (
                  <li key={disciplina} className="bg-gradient-to-tr to-azul_1 from-azul_2 text-branco_2 hover:to-azul_2 hover:from-azul_3 transition-all hover:scale-[1.025] rounded-xl shadow-xl p-4">{disciplina}</li>
                ))
            )}
          </ul>
        </div>
